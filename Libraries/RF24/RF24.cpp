...
#ifdef ARDUINO
  SPI85.setBitOrder(MSBFIRST);
  SPI85.setDataMode(SPI_MODE0);
  SPI85.setClockDivider(SPI_CLOCK_DIV4);
#endif
...
uint8_t RF24::read_register(uint8_t reg, uint8_t* buf, uint8_t len)
{
  ...
  csn(LOW);
  status = SPI85.transfer( R_REGISTER | ( REGISTER_MASK & reg ) );
  while ( len-- )
    *buf++ = SPI85.transfer(0xff);
...
{
  csn(LOW);
  SPI85.transfer( R_REGISTER | ( REGISTER_MASK & reg ) );
  uint8_t result = SPI85.transfer(0xff);
...
uint8_t RF24::write_register(uint8_t reg, const uint8_t* buf, uint8_t len)
{
  ...
  csn(LOW);
  status = SPI85.transfer( W_REGISTER | ( REGISTER_MASK & reg ) );
  while ( len-- )
    SPI85.transfer(*buf++);
...
uint8_t RF24::write_payload(const void* buf, uint8_t len)
{
  ...
  csn(LOW);
  status = SPI85.transfer( W_TX_PAYLOAD );
  while ( data_len-- )
    SPI85.transfer(*current++);
  while ( blank_len-- )
    SPI85.transfer(0);
  csn(HIGH);
...
uint8_t RF24::read_payload(void* buf, uint8_t len)
{
  ...
  csn(LOW);
  status = SPI85.transfer( R_RX_PAYLOAD );
  while ( data_len-- )
    *current++ = SPI85.transfer(0xff);
  while ( blank_len-- )
    SPI85.transfer(0xff);
  csn(HIGH);
...
uint8_t RF24::flush_rx(void)
{
  ...
  csn(LOW);
  status = SPI85.transfer( FLUSH_RX );
  csn(HIGH);
...
uint8_t RF24::flush_tx(void)
{
  ...
  csn(LOW);
  status = SPI85.transfer( FLUSH_TX );
  csn(HIGH);
...
uint8_t RF24::getDynamicPayloadSize(void)
{
  ...
  csn(LOW);
  SPI85.transfer( R_RX_PL_WID );
  result = SPI85.transfer(0xff);
  csn(HIGH);
...
void RF24::writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
  ...
  csn(LOW);
  SPI85.transfer( W_ACK_PAYLOAD | ( pipe & B111 ) );
  const uint8_t max_payload_size = 32;
